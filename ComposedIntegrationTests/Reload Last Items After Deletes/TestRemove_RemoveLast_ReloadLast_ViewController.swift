...
final class TestRemove_RemoveLast_ReloadLast_ViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        title = "Remove, Remove Last, Reload Last"
    ...
    private func applyUpdate() {
        rootSection.performBatchUpdates { _ in
            rootSection.remove(at: 4)
            rootSection.remove(at: 2)
            rootSection[1] = "Item 0, 1 (updated)"
            rootSection[2] = "Item 0, 3 → 0, 2 (updated)"
        }
    }
}
